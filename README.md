...
Также для быстрого сохранения/получения данных о запросах (кэширование) будет осуществляться с использованием Redis (redox C++ driver, например). Кэш будет периодически очищаться. При этом при добавлении новых relations некоторое время будет возвращаться прежний результат до очищения кэша.
